...
class SystemProfiler(object):
    ...
    def _authenticate(self):
        r = requests.post(
            f"{self.config.api_base_url}/authenticate",
            json={
    ...
    def _stringify_req_body(self, req_body):
        if not isinstance(req_body, dict):
            self.config.log.debug(
                f"[!] Request body is not a dict() object. Cannot stringify."
            )
            return None
    ...
    def _send_api_request(self, uri_path=None, data_payload=None):
        ...
        if self.access_token == None:
            self.config.log.debug(
                f"[!] No API access token available to make API requests."
            )
            return
    ...
    def register_new_report(self, target, api_path):
        ...
        api_response = self._send_api_request(
            uri_path=api_path, data_payload=request_data
        )
    ...
    def fetch_cpu_stats(self, target, api_path):
        ...
        request_data = self._stringify_req_body(
            req_body={
                "cpu_data": cpu_data,
                "target": target,
    ...
    def fetch_network_stats(self, target, network_info_api_path, network_ip_api_path):
        ...
        network_data = self._stringify_req_body(
            req_body={
                "network_data": netwk_data,
                "target": target,
                "report_id": self.report_id,
            }
        )

        ip_data = self._stringify_req_body(
            req_body={"ips": ips, "target": target, "report_id": self.report_id}
        )

        self._send_api_request(
            uri_path=network_info_api_path, data_payload=network_data
        )
    ...
    def fetch_system_stats(self, target, os_api_path, uptime_api_path, users_api_path):
        ...
        os_req_data = self._stringify_req_body(
            req_body={"os": opersys, "target": target, "report_id": self.report_id}
        )

        uptime_req_data = self._stringify_req_body(
            req_body={"uptime": uptime, "target": target, "report_id": self.report_id}
        )

        users_req_data = self._stringify_req_body(
            req_body={"users": users, "target": target, "report_id": self.report_id}
        )
...
def main():
    ...
    sys_profiler.register_new_report(target=target_hostname, api_path="/api/report/new")

    print(
        f"Running System Profiler Report on {target_hostname}. Using Report ID: {sys_profiler.report_id}."
    )

    sys_profiler.fetch_cpu_stats(target=target_hostname, api_path="/api/cpu/info")

    sys_profiler.fetch_disk_stats(target=target_hostname, api_path="/api/disk/info")

    sys_profiler.fetch_memory_stats(target=target_hostname, api_path="/api/memory/info")

    sys_profiler.fetch_network_stats(
        target=target_hostname,
        network_info_api_path="/api/network/info",
        network_ip_api_path="/api/network/ip",
    )

    sys_profiler.fetch_system_stats(
        target=target_hostname,
        os_api_path="/api/system/os",
        users_api_path="/api/system/users",
        uptime_api_path="/api/system/uptime",
    )

    sys_profiler.fetch_process_stats(
        target=target_hostname, api_path="/api/cpu/processes"
    )
